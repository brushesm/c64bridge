...
2. Decide on the rendering path: `c64_graphics` (op `render_petscii_art`) for generated PETSCII scenes, `c64_graphics` (op `render_petscii_text`) for text layouts, or `c64_graphics` (op `render_sprite`) for sprite previews. Outline the plan before executing.
